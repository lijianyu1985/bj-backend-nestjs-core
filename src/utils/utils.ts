...
export function populate(population: string, query: any) {
  if (population) {
    ...
    (populationItems || []).map(async (populationItem) => {
      ...
      if (populationItemSplits.length === 2) {
        query.populate(populationItemSplits[0], populationItemSplits[1]);
      } else {
        query.populate(populationItemSplits[0]);
      }
    });
  }
  return query;
